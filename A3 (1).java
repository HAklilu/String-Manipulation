 ...
 * 
 * We thought this was a nice assignment. I liked how it allowed us to go 
 * into various classes' api and learn about various functions and how to use
 * them. 
 *
 *  */
...
public class A3 {
	 ...
	 * StringBuilder and a discussion of why it is better to use StringBuilder for
	 * this purpose.
	 *
	 ...
	 * false.<br>
	 * If s has an even number (> 0) of chars, there are two middle chars, so return
	 * <br>
	...
	public static String protectLittles(String s) {
		 ...
		 * from the rest, but only letters in 'a'..'z'. There are other lower-case
		 * letters!
		 *
		 * 2. In the fourth example, s = "å", 'å' is lower-case letter but it is NOT a
		 * character in a..z, so it is NOT preceded by a capital.
		 *
		 * If this isn't working for you, you may be using Eclipse on a Windows 10
		 * computer, and the wrong Text File coding is being used. Check that you are
		 * using the right one by using menu item Preferences -> General -> Workspace
		 * and selecting UTF-8.
		 *
		 ...
		 * example, the internal representation of 'a' is 97, but do NOT use magic
		 * number 97 in the method body; instead, use 'a'.
		 */

		for (int i = 0; i < s.length(); i++) {
			if (s.charAt(i) >= 'a' && s.charAt(i) <= 'z') {
				s = s.substring(0, i) + s.substring(i, i + 1).toUpperCase() + s.substring(i);
				i++;
			}
		}
		return s;
	...
	public static String putCapsLast(String s) {
		...
		return (other.append(upper)).toString();
	...
	public static boolean moreThan1(String s, String s1) {
		...
		if (s.contains(s1)) {
			return (s.indexOf(s1) - s.lastIndexOf(s1)) != 0;
		}
		return false;
	}
	 ...
	 * Return a String that contains k copies of the character. <br>
	 *
	 *
	 * Examples: <br>
	 * expand(" b0 ") is "" <br>
	...
	public static String expand(String s) {
		...
		return digit.repeat(Integer.valueOf(s.substring(1)));
	}
	...
	public static boolean areAnagrams(String s, String t) {
		 ...
		 * classes String and Arrays --look them up. Hint: how can a sequence of
		 * characters be uniquely ordered? You might need to first convert the string
		 * into an array of characters and then use methods in class Arrays.
		 */
	...
	// TODO 7
	/*
	 * This ToDo is worth 3/100 points of this assignment. In the comment on lines
	 * 5..10, fill in your netid(s) and tell us what you thought about this
	 * assignment. Then, on line 45, put in how much time you spent on this
	 * assignment
	 */
